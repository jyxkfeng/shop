...
$(document).ready(function () {
    ...
    $('#submit_ok').bind("click", function () {
        ...
        //如果购物车内有预售商品不能提交普通商品进入购物车
        $.ajax({
            url: "../../member/Ajax/GateWay.aspx",
            data: { "action": "RETRIEEVALCART", "type": "0", "userName":username, "shopCode":shopID, "goodsNO": "" },
            async: false,
            success: function (res) {
                if (res == "false") {
                        showBox("请先结算购物车内预售商品，再购买该商品！");
                    isCanbuy = res;
                }
            }
        });
            ...
            if (num >= 1) {
                if (num <= nummax) {
                    if (num <= purchaseSum || purchaseSum == 0) {
                        ...
                        showBox("此商品限购，您最多购买" + purchaseSum + "件");
                    }
                } else {
                    showBox("库存不足");
                }
