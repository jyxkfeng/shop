...
$(function () {
    ...
    //alert content text 
    $("#order-receive , [data-id='order-receive']").on("click", function (event) {
    ...
    //alert prolong receive
    $("[data-id='prolongreceive']").on("click", function (event) {
        event.preventDefault();
        box_frame('确认延长收货时间？每笔订单只能延长一次（3天）哦！')
        $(".btn-div").delegate("#alert-2btm-close", "click", function (event) {
            event.preventDefault();
            $("#alert-box-2btn").detach();
        })
    ...
    //alert prolong check
    $("[data-id='prolongcheck']").on("click", function (event) {
        event.preventDefault();
        box_frame('确认延迟提货时间？每笔订单只能延长一次哦！')
        $(".btn-div").delegate("#alert-2btm-close", "click", function (event) {
            event.preventDefault();
            $("#alert-box-2btn").detach();
        })
    });
    //alert wechat address verify
    $("#wechataddress-verify").on("click", function (event) {
        event.preventDefault();
        box_frame("v5shop.com.cn要更您的收货地址，是否允许？");
        $(".btn-div").delegate("#alert-2btm-close", "click", function (event) {
            event.preventDefault();
            $("#alert-box-2btn").detach();
        })
    ...
    $("#address-list li").on("click", function () {
        $(this).parent().find(".icon-success").hide();
        $(this).find(".icon-success").show();
    })
    $("#address-list li .div-simply").on("taphold", function (event) {
        event.preventDefault();
        $(".alert-addressoperate-mask").show();
        $(".alert-addressoperate-c").show();
    })
    $(".alert-addressoperate-mask").on("tap", function () {
        $(".alert-addressoperate-mask").hide();
        $(".alert-addressoperate-c").hide();
    });
    //add account
    $("#acount-select").children("div").on("click", function () {
        if (($(this).children("input").prop("checked") == true) && ($(this).children("input").val() == "alipay")) {
            $("#blank-div").hide();
            $("#acount-content").find("input[name='name']").attr("placeholder", "真实姓名");
            $("#acount-content").find("input[name='account']").attr("placeholder", "账号");

        }
        if (($(this).children("input").prop("checked") == true) && ($(this).children("input").val() == "blank")) {
            $("#blank-div").show();
            $("#acount-content").find("input[name='name']").attr("placeholder", "持卡人");
            $("#acount-content").find("input[name='account']").attr("placeholder", "卡号");
        }
    });
    //ios4 fixed
    ...
    if (isIOS) {
        $('form').on('focus', 'input , textarea ,select', function () {
            $('.tab-bar').addClass('relative');
            $('form').removeClass('mt-55')
            $('form').addClass('mt-15')
        }).on('focusout', 'input , textarea ,select', function () {
            $('.tab-bar').removeClass('relative');
            $('form').addClass('mt-55')
        });
    }
    //pact alert
    $("#pact-a").on("click", function (event) {
        event.preventDefault();
        $("#alert-box-pact").fadeIn();
    });
    $("#pact-close,#btn-agree").on("click", function (event) {
        event.preventDefault();
        $("#alert-box-pact").fadeOut();
    });
    //deposit money	select
    $("[data-deposit='deposit_money'] .columns").on("click", function () {
        $("[data-deposit='deposit_money'] .columns").removeClass("current");
    ...
    //personer info
    $("[data-person='person_info_other_title']").on("click", function () {
        $("[data-person='person_info_other_title']").children(".arrow").toggleClass("arrow-rotate")
        $("[data-person='person_info_other_c']").slideToggle();
    });

    //extract
    $("[data-toggle=extract-style]").on("click", function () {
        $("[data-toggle=extract-style] .icon-radius").removeClass('checked');
        $(this).children("a").children(".icon-radius").addClass("checked")
    });

    //sign
    $("[data-toggle=show-sign]").on("click", function () {
        $(".mask-sign").show();
        $("#sign-modal").show();
    });
    $("[data-toggle=close-sign]").on("click", function () {
        $("#sign-modal").hide();
        $(".mask-sign").hide();
    });

    //mytreasure
    $("[data-toggle='show-more']").on("click", function () {
        $(".alert-treasure-num").show();
        $("#alert-mask").show();
    });
    $("[data-toggle='close-more'],#alert-mask").on("click", function () {
        $(".alert-treasure-num").hide();
        $("#alert-mask").hide();
    });

    //dl.sub-nav.nav-b5 dd
    var leg = $("dl.sub-nav.nav-b5 dd").length;
    var wid = "100" / leg;
    $("dl.sub-nav.nav-b5 dd").css("width", wid + "%");
});
//share toggle
function share_foot() {
    $(".mask").fadeToggle();
...
(function () {
    ...
    //member no trade	
    function member_notrade(obj) {
        $(".member-notrade-detail").slideUp();
        if ($(obj).parents().next(".member-notrade-detail").css("display") == "block") {
            $($(obj).parents().next(".member-notrade-detail")).slideUp();
        }
        else {
            $($(obj).parents().next(".member-notrade-detail")).slideDown();
        }
    }
    window.member_notrade = member_notrade;
